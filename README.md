...
DocSearch AI는 사내 문서를 업로드하고, 로컬 추론 서버 기반 RAG로 검색과 질의응답을 제공하는 온프레미스 문서 검색 플랫폼입니다.
...
현재 기준선에는 서비스 경계, 상태 확인 엔드포인트, 로컬 실행 설정, CI, API Key 기반 워크스페이스 인증, 문서 업로드, 개발용 인프로세스 인덱싱 큐, 청킹과 임베딩 파이프라인 경계가 포함되어 있습니다.

다음 단계에서는 Redis 기반 워커 소비 루프, Qdrant 저장, 검색, 리랭킹, 출처 포함 답변 흐름을 순차적으로 추가합니다.
