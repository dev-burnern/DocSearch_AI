...
describe("createDocumentApiClient", () => {
  ...
  it("문서 삭제 API에 API Key와 문서 ID를 전송한다", async () => {
    const fetcher = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          document_id: "doc-1",
  ...
  it("문서 재인덱싱 API에 API Key와 문서 ID를 전송한다", async () => {
    const fetcher = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          document_id: "doc-1",
    ...
    const response = await client.reindexDocument({
      apiKey: "local-dev-key",
      documentId: "doc-1",
    });
    ...
    expect(response.indexing_job_id).toBe("job-2");
